...
- **`isRetryable` gates every *Reintentar*, not just the sheet's**, and it is asked in **one** place. The rule — a control known in advance to fail is not a control — is one rule, so `retryAction(err, onAct)` in `useListItems` reads the status off the `ApiError` (0 when there is none) and returns `undefined` when it is not retryable; call sites pass the error rather than re-deciding a status apiece. Seven sites each special-casing one status is how «Reintentar» ended up on a 404: the item was deleted on another phone, this screen is up to five seconds stale, and every press repeats the same request for the same answer. Where a status has a truer sentence than «no se pudo», say that instead — «el producto ya no existe» is already the sheet's words for the same fact. And a **404 on a delete is not a failure at all**: the row is gone, which is what the tap asked for, so the optimistic removal stands and nothing is said
