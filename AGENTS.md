...
- **`isRetryable` gates every *Reintentar*, not just the sheet's.** The rule — a control known in advance to fail is not a control — is one rule, so a toast that offers a retry has to ask the same question `UnsentChangesSheet` asks: read the status off the `ApiError` (0 when there is none) and attach the action only when `isRetryable(status)`. Unconditional, a 404 from a product a flatmate deleted mid-sheet offers a button that can only 404 again. Where the status has a truer sentence than «no se pudo», say that instead — «el producto ya no existe» is already the sheet's words for the same fact
