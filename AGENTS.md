...
- **The rule about `main` is wider than the deny that enforces it** — writing there by any other route (a redirect, `sed -i`, an interpreter one-liner, a subagent) is caught by the Stop hook at turn end instead. Committing on `main` and `git stash` are caught by nothing at all, and are still forbidden.
- **`--no-verify` and `LEFTHOOK=0` are denied** — fix the failing hook instead of skipping it.
- **A turn does not end on a lint failure** — changed Python and TypeScript are re-checked when Claude Code tries to stop, and the turn is continued to fix them. One continuation, not a loop; lefthook is the backstop at commit time. A dirty `main` shares that one continuation and warns the user on the way out, so leaving it dirty costs lint its retry.
