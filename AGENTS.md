...
- The dashboard avatar opens the `23a` settings sheet (`components/SettingsSheet.tsx`) — the single home for the push toggle, the Siri shortcut key (issued lazily on open, shown masked, regenerated behind a confirm sub-state), install, feedback, and sign-out. Don't add a second surface for any of these. The footer prints `__APP_VERSION__`, injected from package.json in `vite.config.ts`
