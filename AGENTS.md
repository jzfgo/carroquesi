...
- **The rule about `main` is wider than the deny that enforces it** — writing there by any other route (a redirect, `sed -i`, an interpreter one-liner, a subagent) is caught at turn end instead. Committing on `main` and `git stash` are caught by nothing at all, and are still forbidden.
