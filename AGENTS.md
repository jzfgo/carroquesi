...
- The item row (`components/ItemCard.tsx`, `30a`/`33a`) has **three states, two voices**: pending is an instruction in the written face (name, qty, brand/store meta — never a price); in-cart is derived, not stored (`purchased && isTripOpen(purchase_ends_at)`), cart glyph on `--tinta-0`; bought is a mono record with the amount in the right-hand column in `--ink-1` — only the tick is green, and **no strikethrough anywhere**. Two touch targets per row: the circle toggles, the row body opens `ItemActionSheet`, which is the single home for per-field editing (qty/brand/stores/price) — the row carries no chips, avatar, or menu button of its own. Non-today purchase lines carry the re-buy disc (wired to the JAV-128 clone). The pending sheet groups items under store headers by `storeKey()`, labelled with the registry display name; the circle renders dashed while offline
