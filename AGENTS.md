...
- **There is no offline write queue, and the app is read-only without a signal.** Every mutation in `useListItems` opens with `if (isOfflineNow()) return` — the instantaneous `navigator.onLine` read, not the hook's state, because a control can be pressed between the `offline` event and React drawing it disabled. Adding a mutation means adding that guard *and* disabling the control that calls it. See ADR-013 and `docs/superpowers/specs/2026-07-31-remove-offline-queue-design.md`
- **The condition is said once, above the router.** `OfflineBand` mounts in `App` outside every screen's chrome, so no sheet covers it and no scroll carries it off. Because it cannot be hidden, **every guard is silent** — a toast per refused tap would be the same fact a third time over a bar already saying it. Nothing below the router may state it again: `ListNotice`, `DashboardScreen`'s banner and two sheets each used to, in their own words, and only while their own surface was visible
- **Disable what writes, not what leads to a write.** Opening an item, opening a tag editor, filling a field from a suggestion, dismissing a due suggestion and carrying a scan into the input bar are reads or local state, and all stay live — refusing them makes the list less useful in the aisle for nothing. Where a sheet writes straight through with no local draft (`StoreEditSheet`), the refusal goes in the mutator too, because the keyboard reaches it without the button
- **`navigator.onLine` is `true` on a captive portal and in an aisle with wifi but no route.** So a write can still be attempted and still fail, and that write is lost — accepted knowingly, recorded as limit 1 of the removal spec. It is the one case that still speaks, and it says «no se pudo…», which is a different sentence from the band's
- **A write whose caller mutates UI around the call cannot delegate its refusal.** `handleSavePrice` closes its sheet before awaiting and `handleDeletePrice` closes it on resolve, so a guard that returns cleanly reads as success to both. Those two refuse in `ListScreen`, ahead of the state change; the hook keeps a backstop underneath
- Temp ids (`lib/tempId.ts`) are about **latency, not connectivity**: an add paints its row before the POST answers. They outlived the queue. A row under a `tmp-…` id is still fully interactive while its add is merely slow, and no component guards on it — see JAV-97
- **An undo is shown only once the write it undoes has settled** — the server's answer, and only that now: a write that never reached the server has nothing to undo, and offering one would be a control known in advance to fail. Offered earlier, the inverse write can overtake the one it reverses. Undo and retry go back through the same `useListItems` mutation the tap used; a fourth write path is how the reconcile guard gets bypassed with nothing going red
