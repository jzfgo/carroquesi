...
- **`isRetryable` gates every *Reintentar* on a **write**, and it is asked in **one** place.** The rule — a control known in advance to fail is not a control — is one rule, so `retryAction(err, onAct)` in `useListItems` reads the status off the `ApiError` (0 when there is none) and returns `undefined` when it is not retryable; call sites pass the error rather than re-deciding a status apiece. Seven sites each special-casing one status is how «Reintentar» ended up on a 404: the item was deleted on another phone, this screen is up to five seconds stale, and every press repeats the same request for the same answer. Where a status has a truer sentence than «no se pudo», say it — `refusalMessage` is **item-scoped** (a 404 on `addItem` is about the list, not the product), while the button rule is universal. A **404 on a delete is not a failure at all** on either path: the row is gone, which is what the tap asked for, so the optimistic removal stands, the drain counts the op as *sent*, and nothing is said. **403 is not retryable** even though 401 is — a fresh token answers a 401, and every 403 this backend raises is a standing fact about the caller. Retries on a failed **read** (`ItemList`, `DashboardScreen`, `InviteScreen`, `ListMembersSheet`) are outside this rule: a refetch is idempotent, so the objection is weaker — but a list its owner deleted 404s every load, and that button is as permanent as any of these were
