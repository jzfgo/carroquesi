...
`frontend/firebase.json` serves `/assets/**` with `Cache-Control: public, max-age=31536000, immutable`. That is safe for exactly one reason: everything Vite emits into `dist/assets/` is content-hashed, so the URL changes whenever the bytes do.
...
- **A `<Toast>` render site must pass `action`.** All three do now (`ListScreen`, `ListMembersSheet`, `DashboardScreen`) — two did not, so an action built by `showToast` was silently dropped, and `Toast` also fell back to the 3 s bare window because it derives that from `action` being present. A test that asserts only the *message* passes either way: assert the button too, both that it is there when the status is retryable and that it is not when it is not
