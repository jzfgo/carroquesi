...
The remaining gotchas (ports, dev-auth defaults, browser matrix) are in `frontend/AGENTS.md`.
...
All known flags and defaults live in the registry in `backend/app/services/feature_flags.py`; a `user_features.feature` value that isn't a registry key is invalid. Adding, granting, and revoking flags is covered in `backend/AGENTS.md`.
...
- The release procedure itself — version bump, changelog regeneration, PR, and post-merge tagging — lives in the `release` skill. Invoke `/release`; don't reconstruct the steps by hand.
