...
- **The turn will not end while the `main` checkout is dirty** — the deny above only sees Edit and Write, so a shell redirect, `sed -i`, an interpreter one-liner, or a subagent gets past it. The Stop hook therefore checks the *effect* instead, and no phrasing evades it. Two routes still do, because they leave the checkout clean: committing on `main`, and `git stash`. Nothing gates those — don't read the guards as covering them.
- **`--no-verify` and `LEFTHOOK=0` are denied** — fix the failing hook instead of skipping it.
- **A turn does not end on a lint failure** — changed Python and TypeScript are re-checked when Claude Code tries to stop, and the turn is continued to fix them. It forces one continuation, not a loop; lefthook is the backstop at commit time. The dirty-`main` check shares that one-continuation floor, but unlike lint it still warns the user on the floored pass, and it re-fires every turn until `main` is clean — so a chronically dirty `main` spends the continuation lint would have used.
