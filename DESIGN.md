...
- **A visual change does not reliably break the committed Playwright
  baselines.** Measured on the strikethrough removal: it moved only the four
  `item-purchased-*` files, by about 75 pixels each, which is inside the budget
  the suite allows. Every test stayed green while those four quietly stopped
  describing the screen. So when a rule goes, delete the baselines it touched
  and then run `just frontend update-snapshots`, which runs the container every
  committed baseline came out of. Deleting first is the whole trick: the command
  rewrites only what failed, and nothing here fails, so on its own it reports
  success and changes nothing. A missing baseline is always written. Never
  regenerate on your own machine, whatever it runs. Linux is the trap rather
  than macOS: it writes the same `-linux.png` CI expects, so a native baseline
  is accepted and quietly disagrees.
