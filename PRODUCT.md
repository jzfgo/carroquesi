...
4. **Designed for the aisle, not the desk.** Installable PWA that opens and reads without a signal, barcode scanning, and push notifications — for a phone held one-handed in a supermarket with poor signal.
...
- **Connectivity is unreliable by assumption, and the app is read-only without it.** A list opens and reads from cache with no signal; every write is refused at the point of the tap, with one band above the router saying why and the controls that write drawn as unavailable. There is no write queue — it was removed deliberately, and what that costs is written down in `docs/decisions/013-no-offline-write-queue.md`.
...
3. **Refuse a write rather than pretend it landed.** Without a signal the write does not happen and the app says so before the tap, not after. Never show a control that cannot succeed, and never report a write as done when it was not. This replaces «never lose a write», which the offline queue existed to keep and which was dropped with it (ADR-013).
