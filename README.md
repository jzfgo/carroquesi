...
- **Fonts:** vendored, not linked. The five faces are served from the app's own origin and precached with everything else, so the app opens offline in the type it was designed in and no page load asks a CDN what it looks like. `frontend/scripts/fetch-fonts.py` is what refreshes them.
...
| [lefthook](https://lefthook.dev/)                                            | Git hooks (lint, format, secrets on commit)                          | `brew install lefthook`                                     |
