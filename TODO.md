...
- [ ] **List filtering** — allow users to filter both the unpurchased and purchased sections by free-text name and/or sigil values (`#brand`, `@store`); a combination of filters should AND together (e.g. `@Mercadona #Danone` shows only Danone items from Mercadona); filter state should be ephemeral (cleared on navigation) and not affect the underlying data
