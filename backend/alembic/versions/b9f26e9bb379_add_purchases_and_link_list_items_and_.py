...
Revision ID: b9f26e9bb379
Revises: 7005338bb031
Create Date: 2026-07-29 00:14:03.788699
...
TRIP_TIMEZONE and the tear-off arithmetic are duplicated from
app.services.trips rather than imported from it. A migration must keep meaning
what it meant the day it ran, even if the app's trip-boundary policy changes
later — importing the live module would let a future policy change silently
rewrite what this backfill computed.
"""
...
revision: str = "b9f26e9bb379"
down_revision: str | Sequence[str] | None = "7005338bb031"
branch_labels: str | Sequence[str] | None = None
depends_on: str | Sequence[str] | None = None

# See module docstring: deliberately not `from app.services.trips import ...`.
...
def upgrade() -> None:
    ...
    # this migration otherwise. Supported on SQLite 3.8+ and Postgres, so no
    # dialect branch is needed the way list_items_store_to_stores.py needed one.
    op.create_index(
    ...
    # FK attached to the column (inline sa.ForeignKey vs. a later
    # op.create_foreign_key makes no difference — both go through the same
    # add_constraint path), and SQLite's dialect impl refuses that outside
    # batch mode. Batch mode was rejected here rather than reached for: batch
    # recreates the table from *reflection*, and 7005338bb031's docstring
    # already recorded that receipt_scans.receipt_at reflects as a distinct
    # DATE type on SQLite — wrapping this add in
    # batch_alter_table("receipt_scans") would walk straight into the same
    # type-drift corruption that migration's docstring warns about. So on
    # SQLite these two columns simply don't get an FK constraint at the DB
    # level. That's a real, deliberate gap, not an oversight:
