...
PRE_PURCHASES_REVISION = "88134f3d9290"  # the revision just before ours
PURCHASES_MIGRATION = (
    BACKEND_DIR / "alembic" / "versions" / "45e4c812fc12_add_purchases_and_link_list_items_and_.py"
)
...
def alembic_config(db_url: str, monkeypatch: pytest.MonkeyPatch) -> Config:
    ...
    # reads the *settings singleton*, not whatever is set on the Config object
    # here — setting it on the Config alone is silently ignored. Patch the
    # singleton instead.
...
def _insert_household(conn: sa.Connection, list_id: str) -> None:
    ...
    conn.execute(
        ...
        {"id": f"user-{list_id}", "uid": f"uid-{list_id}", "email": f"{list_id}@x.com", "now": now},
    )
    conn.execute(
        sa.text(
        ...
        {"id": list_id, "name": "Household", "owner_id": f"user-{list_id}", "now": now},
    )
...
def _insert_purchased_item(
    ...
    Using the ORM models here would be wrong: the point of this test is that
...
def test_schema_matches_models_after_upgrade(alembic_config: Config, db_url: str) -> None:
    ...
    differently, and the purchases migration's own list_items/receipt_scans
    FKs are intentionally absent on SQLite (see that migration's upgrade()
    comment) — so that is explicitly out of scope here.
    """
    command.upgrade(alembic_config, "head")
    ...
    # Purchase.__table_args__) — a migration that forgot this index would
    # leave production unprotected while every other assertion here still
    # passed.
    index_names = {ix["name"] for ix in inspector.get_indexes("purchases")}
    assert "uq_purchases_open_per_list" in index_names


def test_migration_helper_agrees_with_the_live_boundary_rule() -> None:
    """The migration duplicates the tear-off arithmetic rather than importing
    app.services.trips (a migration must keep meaning what it meant the day
    it ran). Duplication is only safe if the duplicate agreed with the live
    rule at the moment it shipped — so compare the two directly, pinned to
    Madrid, across instants that exercise both CET and CEST and the DST
    switch days themselves.
    """
    from app.services.trips import tears_off_at_for

    spec = importlib.util.spec_from_file_location("purchases_migration", PURCHASES_MIGRATION)
    ...
    instants = [
        datetime(2026, 1, 15, 12, 0, 0),  # winter, CET (UTC+1)
        datetime(2026, 7, 28, 21, 30, 0),  # summer, CEST (UTC+2)
        datetime(2026, 7, 28, 22, 30, 0),  # already the next Madrid day
        datetime(2026, 3, 29, 0, 30, 0),  # spring-forward day
        datetime(2026, 10, 25, 0, 30, 0),  # fall-back day
        datetime(2026, 12, 31, 23, 30, 0),  # year boundary
    ]
    for instant in instants:
        assert migration._tears_off_at_for(instant) == tears_off_at_for(instant, madrid), instant


def test_backfilled_tears_off_at_matches_the_live_trip_boundary_rule(
    alembic_config: Config, db_url: str
) -> None:
    """Same agreement, proven end to end: the value the backfill actually
    wrote must be what the live rule computes for Madrid. A backfill that
    computed the *same* day's midnight instead of the *next* one would still
    group items identically and pass every grouping test here, while leaving
    `tears_off_at` values no equality lookup could ever match.
    """
    from app.services.trips import tears_off_at_for
    ...
    expected = tears_off_at_for(datetime.fromisoformat(purchased_at), ZoneInfo("Europe/Madrid"))
...
def test_open_per_list_index_is_unique_and_partial(alembic_config: Config, db_url: str) -> None:
    ...
    guarantee (see Purchase.__table_args__).
    ...
    # and it's the legitimate closed-trip-plus-fresh-trip-same-day case that
...
def test_downgrade_reverses_cleanly(alembic_config: Config, db_url: str) -> None:
    command.upgrade(alembic_config, "head")
    command.downgrade(alembic_config, PRE_PURCHASES_REVISION)
