...
This introduces a new, non-Firebase auth path — a static per-user API key — which is deliberately built as shared infrastructure: JAV-8 (MCP server) is a separate, later effort that will authenticate with the same `X-Api-Key` mechanism this spec builds. Nothing here is Siri-specific at the backend layer; only the `.shortcut` plist generation and the frontend Apple-only UI are.
