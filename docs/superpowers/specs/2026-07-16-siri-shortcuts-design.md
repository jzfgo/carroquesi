...
**What broke:** the original spec's "Correction to the ticket's wording" section (below) predicted that a plain, unsigned `.shortcut` made only of URL-request actions would import without friction. Real-device testing disproved this: modern iOS hard-blocks *all* unsigned `.shortcut` imports (Files/Safari/AirDrop), with no user-facing toggle — this isn't a warning to click through, it's a wall. The distinction that mattered wasn't "scripting actions vs. URL actions," it's "signed vs. unsigned," full stop.
