...
**What broke:** the original spec's "Correction to the ticket's wording" section (below) predicted that a plain, unsigned `.shortcut` made only of URL-request actions would import without friction. A real-device test in an earlier session suggested otherwise — an unsigned file failed to import where a signed one succeeded — and we pivoted to signing on that basis.

> **Caveat (added 2026-07-17, on review):** that conclusion is weaker than the wording above implies, and this paragraph originally overstated it. There is **no primary record** of the device test (no screenshot/log/steps) — only this prose summary. A later check of public and Apple documentation did **not** corroborate a "hard block on all unsigned imports": iOS 15 removed the old "Allow Untrusted Shortcuts" toggle, and [multiple](https://www.howtogeek.com/447421/how-to-allow-untrusted-shortcuts-on-iphone-and-ipad/) [sources](https://www.imore.com/how-allow-untrusted-shortcuts-and-find-trusted-shortcuts-creators) describe outside-Gallery shortcuts now showing an "Add Shortcut" screen rather than a wall; [Apple's own support page](https://support.apple.com/guide/shortcuts/advanced-shortcuts-settings-apdfeb05586f/ios) documents gating around *script/JavaScript* execution, not a signature gate, and doesn't cleanly separate signed iCloud-shared shortcuts from raw `.shortcut` files. Note also that transport (http vs https) is **orthogonal** to whether a downloaded file's signature is checked — it only affects the `shortcuts://import-shortcut?url=` deep-link fetch. So the load-bearing fact is the positive one below (**signed shortcuts import cleanly — verified**), not a definitive "unsigned is blocked." We ship signed because it's the path we confirmed works, not because unsigned was proven impossible.
