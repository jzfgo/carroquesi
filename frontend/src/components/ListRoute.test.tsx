...
describe('ListRoute', () => {
  ...
  it('shows not-found error on 404', async () => {
    vi.mocked(api.getList).mockRejectedValue(
      new api.ApiError(404, 'Not found'),
    );
    render(<ListRoute />);
    await waitFor(() =>
      expect(screen.getByText('Lista no encontrada.')).toBeInTheDocument(),
    );
  });
  ...
  it('shows forbidden error on 403', async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByText('No tienes acceso a esta lista.'),
      ).toBeInTheDocument(),
    );
  });
  ...
  it('shows generic error on unknown failure', async () => {
    ...
    render(<ListRoute />);
    await waitFor(() =>
      expect(screen.getByText('Error al cargar la lista.')).toBeInTheDocument(),
    );
  });
