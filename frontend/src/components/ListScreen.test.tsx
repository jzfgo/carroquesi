...
describe('header variant preview toggle', () => {
  afterEach(() => {
    window.history.replaceState(null, '', '/')
    document.querySelector('meta[name="theme-color"]')?.remove()
  })

  it('defaults to the board variant', () => {
    const { container } = render(
      <ListScreen listId="l1" listName="Mercado" listOwnerId="u1" />,
    )
    expect(container.querySelector('.list-screen')).toHaveAttribute(
      'data-header-variant',
      'board',
  ...
  it('?header=paper selects the paper variant and leaves theme-color alone', () => {
    window.history.replaceState(null, '', '/?header=paper')
    ...
    expect(meta.getAttribute('content')).toBe('#EEF1F5')
  })

  it('board variant tints theme-color with the board and restores it on unmount', () => {
    const meta = document.createElement('meta')
    meta.setAttribute('name', 'theme-color')
    meta.setAttribute('content', '#EEF1F5')
    document.head.appendChild(meta)
    ...
    // Light salvia — the setup matchMedia resolves the system scheme to light.
    expect(meta.getAttribute('content')).toBe('#a9b8a5')
    unmount()
    expect(meta.getAttribute('content')).toBe('#EEF1F5')
  })
})
