...
### The glyphs are vendored, and a font refresh is a suite-wide change

`src/fonts.css` declares every face and `src/assets/fonts/` holds the bytes, both checked in. Nothing in the app requests `fonts.googleapis.com` or `fonts.gstatic.com`, so the letterforms in every committed baseline are files in this repo rather than whatever a CDN served the day the capture ran.

That matters here because of what the alternative costs. While the fonts came from a CDN, the container was the one machine whose output the rest of the baselines agreed with — but only for as long as that CDN kept serving it the same bytes. Google re-cuts a family and bumps its version directory (`geist/v5`, `caveat/v23`) when it does, and every baseline in the suite goes stale at once, with no commit to blame. If the re-cut is a re-hinting rather than a redraw, each capture moves less than 250, nothing fails, and the suite goes on asserting glyphs the app has stopped drawing. Nothing in the repo can tell you it has happened.

Vendoring makes that a diff instead of a silent event, and a changed `.woff2` **is** the notice: the letterforms moved, so the baselines have to be regenerated in the same PR. That is the half vendoring does not do by itself — it makes the change visible, not safe. Treat a font refresh as an app-wide rendering change, which is what the section above is about.

Two traps if you ever refresh them. Google answers `css2` by **User-Agent** and serves a different cut of every family to Android — same outlines, hinting tables stripped — so fetching with the wrong UA moves the mobile baselines and nothing else. And `≈`, beside a converted price in `PriceHistoryBlock` and the running total in `LogPurchaseSheet`, is outside every subset Google serves, `latin` included: it comes from a system font, and it is the one glyph in the suite whose shape is still the container's to decide.
