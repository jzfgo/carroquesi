...
They no longer do. `src/fonts.css` declares every face and `src/assets/fonts/` holds the bytes, both checked in; nothing in the app requests `fonts.googleapis.com` or `fonts.gstatic.com` any more. This section stays because what it described was real, and vendoring is only half the fix.
...
Vendoring turns that from a silent event into a diff. A refresh is a hand job, and the header of `src/fonts.css` is its recipe — but whatever route it takes, a changed `.woff2` in the diff **is** the notice: the letterforms moved, so the baselines have to be regenerated in the same PR. That is the half the vendoring does not do by itself — it makes the change visible, it does not make it safe. Treat a font refresh as an app-wide rendering change, which is what the section above is about.

It has already happened once, on the vendoring itself, and it is worth knowing how it looked. Google answers `css2` by User-Agent and serves a _different cut_ of every family to Android — the same outlines, hinting tables stripped. So the committed baselines had never been one set of letterforms: the `chromium` ones were captured with the desktop cut and the `Mobile-Chrome` ones with the Android cut, and nothing here said so. Vendoring collapses both onto the desktop cut, which moved 25 mobile baselines and no desktop ones. Unifying was the right outcome and self-hosting cannot avoid it, since one origin serves one file to everyone.
